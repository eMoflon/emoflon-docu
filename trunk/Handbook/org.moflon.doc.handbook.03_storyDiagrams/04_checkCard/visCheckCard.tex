...
\texttt{Partition::check} as depicted in Fig.~\ref{ea:sdm_check_start}.
...
(Fig.~\ref{ea:sdm_check_addCard}). This will represent the card the user picked from the learning box. Remember, the binding for this variable is implicitly
...
Fig.~\ref{ea:sdmcheck_att_constraint}, and select the correct \texttt{Attribute} and \texttt{Operator}.
...
\item[$\blacktriangleright$] Similar to how the return value was specified in the previous SDM, set the \texttt{ParameterExpression} to refer to \texttt{guessed},
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=1\textwidth]{ea_extractStoryPattern}
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.48\textwidth]{sdm_promoteCardProjBrowser}
...
for eMoflon diagrams). Paste \texttt{Card} as an \texttt{Instance}, and select \texttt{Save selection as default} so option (2) will be
used next time by default. You should also select \texttt{Hold Ctrl to show this dialog}, so this dialogue doesn't appear every time you use this
...
\texttt{void} to \texttt{LiteralEx\-pression} (Fig.~\ref{ea:sdm_check_literal_exp}). Double click on  the window below to change the value to \texttt{true}. Press \texttt{OK},
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.4\textwidth]{ea_stopNodeLiteral}
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=1\textwidth]{ea_sdmCheckComplete}
