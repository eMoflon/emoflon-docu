...
In order to conduct these transformations, patterns must establish\define{Object \\ Variables}\emph{object variables}. Object variables are, as
...
to the object whose method is invoked, and \texttt{card}, the card object to be removed. In order to assign \texttt{card}, we'll need\define{Parameter
Expression}to use a \emph{parameter expression} to refers to the parameter of the current method.

Patterns must also establish \emph{link variables}\define{Link \\ Variables} to match links in story patterns. These too act as place holders, and
...
unbounded, or a \emph{free variable}.\define{Free \\ Variable}Values for these objects can be both assigned and changed throughout the activity. By declaring
