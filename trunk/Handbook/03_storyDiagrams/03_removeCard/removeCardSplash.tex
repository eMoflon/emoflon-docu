...
These patterns however, must establish\define{Object \\ Variables}\emph{object variables}, place holders for actual objects in a model (i.e., an object
...
be influenced so that, as required for \texttt{removeCard}, the pattern matcher chooses the correct \texttt{card} (passed in as a parameter)?

The \emph{binding state}\define{Binding~State}of an object variable determines how its found. By default, every object variable is \emph{unbound}, or a 
\emph{free variable}.\define{Free \\ Variable}Values for these variables can be determined automatically by the pattern matcher. By declaring a
\emph{bounded}\define{Bounded}object variable however, it will have a fixed value determined from the previous activity nodes.
