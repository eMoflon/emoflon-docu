...
variable \texttt{nextPartition}, of type \texttt{Partition}, and set \define{Binding\\Semantics} its \emph{Binding Semantics} to \texttt{negative}
(Fig.~\ref{fig:sdm_grow_2}). The object variable should be visualised as being cancelled or struck out. % why?
 
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.9\textwidth]{ea_newNac}
...
\item[$\blacktriangleright$] Complete the story pattern so that it closely resembles Fig.~\ref{fig:sdm_grow_3}. Be sure to triple-check your references to make
sure everything will transition in the right direction.
...
\item[$\blacktriangleright$] To complete this SDM, build the assignment to set the size of the \texttt{newPartition}. Go ahead and invoke the corresponding
dialogue to activate the \texttt{:=} operator.
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.5\textwidth]{ea_attributeMethodCall.png}
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=\textwidth]{ea_completeActivityGrowBox.pdf}
...
\fancyfoot[R]{ $\triangleright$ \hyperlink{sec:stringRep}{Next}}
