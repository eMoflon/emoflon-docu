...
repeating the action for all cards in the partition. In SDM, this is \note{For Each} accomplished via a \emph{For Each} story node. A for each story node
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.8\textwidth]{sdm42RAW}
...
\note{Non-Determinism}   Well, the pattern would specify that \emph{a} card should be matched and deleted from the current partition. Note that the \emph{exact}
...
\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.9\textwidth]{sdm47}
  \caption{Complete story pattern with \texttt{[end]} guard.}  
  \label{fig:sdm_end}
\end{center}
\end{figure}

The second point is if we need to destroy the link between \texttt{this} and \texttt{card}.   Would the pattern be interpreted differently if we just destroyed
