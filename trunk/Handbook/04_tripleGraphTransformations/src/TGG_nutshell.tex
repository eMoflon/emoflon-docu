...
via a precondition (LHS) and postcondition (RHS). The key difference is that a TGG rule will always describe how a \emph{graph triple}\define{Graph Triple}
...
key\define{Bijective} idea is that the forward transformation is very often actually \emph{not} injective\define{Injective} and cannot be inverted! A function
can only be inverted if it is \emph{bijective}, meaning it is both \emph{injective} and \emph{surjective}.\define{Surjective}So how can we derive the backwards
transformation?
