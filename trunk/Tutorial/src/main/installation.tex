...
\begin{enumerate}
\item[$\blacktriangleright$] Download and install Eclipse for Modeling ``Eclipse Modeling Tools (includes Incubating components)'' (Fig.~\ref{fig_downloadModelingPackage})\footnote{Tested for Eclipse 3.7 (Indigo).} from:\\ \url{http://www.eclipse.org/downloads/packages/}
\begin{figure}[htbp]
	\centering
  \includegraphics[width=0.65\textwidth]{pics/installationAndSetup/eclipse_modelingpackage.png}
...
project, and refreshing triggers a build process that invokes our
...
\begin{minipage}[b]{0.3\textheight}
  \centering
  \includegraphics[width=\textwidth]{pics/installationAndSetup/standardCodeEngineering}
...
used to communicate with EA and initiate code generation by simply pressing F5
...
This Ecore model is used to drive a code generator that maps the model to Java
...
aspects modelled using SDM (3).  After exporting the model and code generation
