...
\begin{enumerate}
  ...
  \texttt{DictionaryCodeAdapter}, create a \texttt{Trans\-for\-mer} class with the methods and references to the \texttt{Library} class as depicted in Fig.~\ref{fig:moca-DictionaryCodeAdapter}.
...
\end{center}
\end{figure}
\item[$\blacktriangleright$] \texttt{transformFolders(Folder)~:void}
(Fig.~\ref{fig:moca-transformFolders})
%\usepackage{graphics} is needed for \includegraphics
\begin{figure}[!htbp]
\begin{center}
...
\end{center}
\end{figure}
\item[$\blacktriangleright$]  The first SDM \texttt{transformFolder(Folder)~:Library} (Fig.~\ref{fig:moca-transformFolder}) creates a corresponding library with shelves for the given folder, delegating the creation of dictionaries to \texttt{transformNode}.
