...
For making the validation output window visible in EA, choose
...
everything right while modelling your \texttt{MemoryBoxLanguage} in the last
...
\begin{figure}[htbp]
	\centering 
  \includegraphics[width=\textwidth]{pics/memBoxBilder/memBox42} 
	\caption{Output after your first validation}
	\label{fig:first_validation}
\end{figure}

Now we will put some small modelling errors (one bad practise and one syntax
error) into our \texttt{MemoryBoxLanguage} to get familiar with validation and
quick fix features.

Create a new class in the MemoryBoxLanguage diagram by drag\&drop from the
...
apperently not the correct way of deleting \texttt{EClass1}. (However, we will
discuss later how to delete elements in EA correctly.)
...
\texttt{previous}. In the end, ensure that your metamodel resembles cloesly
