...
\item[$\blacktriangleright$] Run the automatically created main class \texttt{TGGMain} in \texttt{LearningBox\-To\-Dictionary\-In\-te\-gra\-tion\-/src} and refresh the folder \texttt{LearningBox\-To\-Dictionary\-In\-te\-gra\-tion/\-instances}. Note that you receive a red output ``Unable to load instances/source.xmi, instances/source.xmi does not exist.'', but this is no problem since we did not intend to do a forward transformation and the backward transformation will be made anyways.
\item[$\blacktriangleright$] Make a copy of \texttt{target.xmi\_BWD.xmi} and rename it to \texttt{source.xmi} (the original file is needed for debugging).
