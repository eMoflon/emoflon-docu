...
end of the EReference (the \texttt{Partition} role). As you can see, the default source is set to the EClass you linked \emph{to}, and the default target
...
Next, you set the \texttt{Multiplicity} value. In the source (\texttt{Partition}) role, you have specified that any target (in our case, \texttt{box}) can have
any positive-sized number of sources. In your target role (\texttt{Box}), you have allowed the creation of up to one target (\texttt{box}) reference
...
boxes). Figure~\ref{fig:sketch_roles} sketches this schematically.
