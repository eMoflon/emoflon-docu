...
\begin{enumerate}
\item[$\blacktriangleright$] \hypertarget{static vis}{LINKED WORD} Open a new project/name it this/double click the .eap file /(note: if you've completed part one, you can complete this section by opening the 'demo' eap file, and working from the demo project root in EA. the following steps are exactly the same, but do be aware that the screenshots may not exactly match. You won't have THIS project root, since it's pre-included in the demo files you previously downloaded.)

{\bf TO DO rewrite/screenshot this entire section again, with the user starting from scratch. NOTE: keep all old files and pictures in case tony doesn't like doing this way. I prefer having the users start from scratch because it is how they would begin their own program.}

\item[$\blacktriangleright$] Open Enterprise Architect (EA) by double-clicking the \texttt{Demo.eap} file it generates. In EA, select \texttt{Demo} and click on the button \texttt{Add a Package} as depicted in Fig.~\ref{fig:new_package}.
...
\begin{figure}[htbp]
	\centering
  \includegraphics[width=0.5\textwidth]{EA_operationRemoveCard}
