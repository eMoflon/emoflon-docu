...
\label{stepThree}
% Should this be put in the summary?
In the next few steps, you'll start to notice some variations between the tasks for the Visual and Text demos.
Unique instructions will be indicated with ?? %{\bf V} 
for Visual, and ?? %{\bf T} 
for textual.
...
% Even if you're only using text, 
...
\item[{\bf V}] So, to sum up. This is Visual.
\item[{\bf T}] This will indicate a textual explaination. I don't really have any just yet, so this will have to do.
\item[$\rhd$] We could also use this
\item[$\lhd$] and this - a bit more elegant
\item[$\RHD$] Oh, there's this one too
\item[$\LHD$] but at first glance you might get confused
