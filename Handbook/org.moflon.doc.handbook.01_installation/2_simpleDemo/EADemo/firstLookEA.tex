...
\item[$\blacktriangleright$] Double-click \entity{Demo.eap} to start EA, and choose \entity{Ultimate} when starting EA for the first time.

\item[$\blacktriangleright$] In EA, select \menuPath{Extensions \menuSep Add-In Windows} (\Cref{ea:validate_dropdown}).
...
This is where you can validate and export your complete project to Eclipse by pressing \entity{All} (\Cref{ea:controlPanel}).
%
\begin{figure}[htbp]
	\centering
...
\item[$\blacktriangleright$] Switch back to Eclipse, choose your metamodel project, and press \entity{F5} to refresh. 
The export from EA places all required files in a hidden folder (\entity{.temp}) in the project.
A new, third project named \entity{org.moflon.demo.doublelinkedlist} is now being created.
Do not worry about the problem markers.

\item[$\blacktriangleright$] The three asterisks signal that the project still needs to be built (\Cref{eclipse:dirty-project}).
%
\begin{figure}[htbp]
    \centering
...
\end{figure}
%
\item[$\blacktriangleright$] Now, right-click \entity{org.moflon.demo.doublelinkedlist} and choose \menuPath{eMoflon \menuSep Build} (or use the shortcut \entity{Alt+Shift+E,B}\footnote{First press \entity{Alt+Shift+E}, release, and press \entity{B}.
By default, most shortcuts eMoflon start with \entity{Alt+Shift+E}.}).

eMoflon now generates the Java code in your repository project.
You should be able to monitor the progress with the green bar in the lower right corner (\Cref{eclipse:build}).
