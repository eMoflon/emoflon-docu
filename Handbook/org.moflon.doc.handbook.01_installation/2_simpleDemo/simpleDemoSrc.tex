...
New definitions or concepts will be \newconcept{italicized}, and any data you're required to enter or select will be given as \entity{command}.} and choose \menuPath{eMoflon} (\Cref{eclipse:openPerspective}).
...
Navigate to \menuPath{\eMoflonMenuButton \menuSep Install Workspace}
\needsUpdate{Please update the screenshot.} (\Cref{eclipse:newMetamodel}).
%
\vspace{0.5cm}
\begin{figure}[htbp]
	\centering
...
Select \menuPath{eMoflon Examples \menuSep Demo (Double-Linked List)}. 

A technical side note concerning Git:
The full source code of eMoflon and all our handbook examples are provided via Git and are hosted on GitHub.\footnote{See \linkto{https://github.com/eMoflon/emoflon-tool/} and \linkto{https://github.com/eMoflon/emoflon-examples}, respectively.}
...
To undo these changes, right-click your working copy again and select \menuPath{Reset\dots}.
Select \menuPath{master} and tick \menuPath{Hard}, then confirm with \menuPath{Reset}.
\end{itemize}


\item
Another button in the new action set is \menuPath{View and configure logging} represented by an \texttt{L} (\eMoflonConfigureLoggingIcon, \Cref{eclipse:logger}).
