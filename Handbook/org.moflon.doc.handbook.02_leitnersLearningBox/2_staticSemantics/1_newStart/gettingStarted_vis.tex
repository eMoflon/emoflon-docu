...
\item  To begin, navigate to ``New Metamodel Project''(\Cref{eclipse:newVisModelButton}) and start a new project named \texttt{Leit\-ners\-Learn\-ing\-Box} (\Cref{eclipse:newVisModel}). 
...
\item
As shown in \Cref{eclipse:OutdatedLabel}, the freshly created project bears the label \textcolor{green}{[outdated]}.
This label indicates that either the contained \filename{eap} file has not been exported yet or that a re-export is necesary (We will explain what \enquote{exporting} means in \Cref{sec:EAExport}).
%
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{eclipse_OutdatedLabel}
...
In EA, select your working set and press the ``Add a Package'' button (\Cref{ea:newPackage}). 
