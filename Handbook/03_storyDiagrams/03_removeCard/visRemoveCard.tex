...
node}\note{Activity Node}. We shall refer to the whole activity diagram simply as an \emph{activity}\note{Activity} that always starts with a start node,
terminates with a \emph{stop node}, and consists of activity nodes connected via \emph{activity edges}\note{Activity Edge}. If you quick created correctly, you
...
The activity node should now have a single \emph{object variable}\note{Object Variable}, \texttt{this}(Fig.~\ref{fig:tool_box}). Object variables are, as the
...
to \texttt{this} (the current partition who owns the method), and assignments to parameters of the \note{Binding State} method that  are specified when invoking
...
Models consist not only of objects, but also of \emph{links}. To match links, one can thus create \emph{link variables}\note{Link Variable}  in story patterns
...
and how to use them. For the moment however, we'll use a \emph{ParameterExpression}\note{Parameter- Expression} which refers to the parameters of the current
...
\fancyfoot[R]{ $\triangleright$ \hyperlink{sec:checkCard}{Next SDM} }
