...
\item[$\blacktriangleright$] Open \texttt{LearningBoxLanguage.eap} in Enterprise Architect (EA) by double-clicking it in Eclipse. Carefully do the
...
this is \emph{removeCard}'s imperative top level diagram. We refer to the whole activity diagram simply as the \emph{activity}, which always starts with a
...
context menu anywhere in the diagram by pressing space.
...
and stop nodes), those that \emph{can} are called story nodes. Statement nodes are just used to invoke an action, such as method execution. We'll encounter this
in a later SDM.
...
assigned to the parameter value of the same name.
...
\texttt{card} is the sole parameter, it will be automatically selected. A \emph{ParameterExpression} is an expression that exclusively accesses
parameter values.

\vspace{0.5cm}

We're nearly done! As you can see, by using several different dialogues, eMoflon employs a simple context-sensitive expression language for specifying  values.
We have intentionally avoided creating a full-blown sub-language, and limit expressions to a few simple types. The philosophy here is to keep things simple and
concentrate on what SDMs are good for -- expressing structural changes. Our approach is to provide a clear and type-safe interface to a general purpose
language (Java) and support a simple \emph{fallback} via calls to injected methods as soon as things get too low-level and difficult to express as a pattern.
