...
\emph{edge guards}\define{Edge Guards}to change this into an \emph{if/Else} construct based on the results of the attribute constraint.
...
to return so instead, we need to edit the stop nodes so they return a\define{LiteralExpression}\emph{LiteralExpression}. This expression type can be used to
