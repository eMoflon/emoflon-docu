...
to retrieve the user's EString input. Press \texttt{Add} and admire your first conditional.
...
this course of action. In other words, eMoflon supports seperating your transformation's pattern layer from its imperative.
...
Another EA gesture\footnote{The other two gestures we have learnt are ``Quick Link'' and ``Quick Create''} you should start to take advantage of here is a good
ol' \emph{Drag-and-Drop} action from the project browserinto a diagram. We can use this action as an alternative to creating new objects (with known types) from
the SDM toolbox.
...
Ctrl + Mouse drag to show this Dialog}, so this dialogue doesn't appear every time you use this action. Don't worry - if you ever need options (1) and (3), 
just hold \texttt{Ctrl} when dragging to invoke the dialogue, then change the settings.
