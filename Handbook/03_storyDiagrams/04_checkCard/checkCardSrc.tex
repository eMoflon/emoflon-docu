...
\define{Edge~Guards}explicit statements on which actions to take when the \emph{if} statement fails or succeeds. To implement this, the \emph{if} statment must
include an \emph{Attribute Constraint},\define{Attribute~Constraint}a non-structural condition that must be satisifed for a story pattern to match. This
