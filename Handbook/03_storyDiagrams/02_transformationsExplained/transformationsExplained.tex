...
(1) is called \emph{graph pattern matching}\note{Pattern Matching}, and this finds occurrences of object(s) $L$ in the model $G$ via an $m$ match.
...
\note{Story\\Pattern} \emph{story pattern} composed of  $Destroy := (L\setminus R)$ in red, $Retain :=  L\cap R$ in black, and $Create := (R\setminus L)$ in 
...
\note{Activity\\Node} is a pattern (as discussed above) with \emph{activity edges} \note{Activity Edges} which link to other nodes to form a control flow. The
...
\fancyfoot[R]{ $\triangleright$ \hyperlink{sec:remCard}{First SDM: Removing a Card} }
