...
complex model transformations consisting of many steps! This can be done through simplified activities, where a single \emph{activity} or \emph{control 
node} is either a single pattern, or made of several (as discussed above) to form a control flow. The entire rule can therefore be viewed as two separate
layers: an imperative layer to define the top-level control flow via activities (ie., if-else statements, loops, etc.), and a pattern layer comprised of story patterns where each activity node specifies (via a graph transformation rule) how the model is to be manipulated.
