...
also want to create the invoking \texttt{Partition} object, \texttt{this} (\Cref{ea:sdm_foreach}). Press \texttt{OK}, and you'll see that a \emph{for each}
...
(\Cref{ea:sdm_complete_control_flow}) however, the goal of \texttt{emptyCards} is not just to remove the link between the selected partition and card, we
...
the \texttt{Binding Operator} to \texttt{Destroy} (\Cref{ea:sdm_bindingOperator}).

\item Complete the story pattern as indicated in \Cref{ea:sdm_end}. Notice that the guard that terminates the looping node has an
